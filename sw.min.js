async function install(){let cache=await caches.open('v29')
...
await Promise.all(cacheNames.filter(cacheName=>{return cacheName!=='v29'}).map(async cacheName=>{return await caches.delete(cacheName);}))}
...
let cache=await caches.open('v29')
