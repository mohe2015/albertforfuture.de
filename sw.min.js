async function install(){let cache=await caches.open('v15')
...
await Promise.all(cacheNames.filter(cacheName=>{return cacheName!=='v15'}).map(async cacheName=>{return await caches.delete(cacheName);}))}
...
let cache=await caches.open('v15')
