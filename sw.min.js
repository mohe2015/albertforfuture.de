async function install(){let cache=await caches.open('v27')
...
await Promise.all(cacheNames.filter(cacheName=>{return cacheName!=='v27'}).map(async cacheName=>{return await caches.delete(cacheName);}))}
...
let cache=await caches.open('v27')
